...
class Filter: NSObject {
    class MKFilter {
        static var xv = [Double](repeating: 0.0, count: numberOfZeros + 1)
        static var yv = [Double](repeating: 0.0, count: numberOfPoles + 1)
    }
}

extension Filter.MKFilter {
    static func processValue(value: Double) -> Double {
        xv[0] = xv[1]
        xv[1] = xv[2]
        xv[2] = xv[3]
        xv[3] = xv[4]
